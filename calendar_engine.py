...
class Assignment:
    ...
    category: str; priority: str; notes: str = ""; completed: bool = False; points: int = 10
...
CREATE TABLE IF NOT EXISTS assignments (
 ...
 completed INTEGER NOT NULL DEFAULT 0, points INTEGER NOT NULL DEFAULT 10,
...
def connect(path: Path) -> sqlite3.Connection:
    ...
    if "points" not in {row[1] for row in db.execute("PRAGMA table_info(assignments)")}: db.execute("ALTER TABLE assignments ADD COLUMN points INTEGER NOT NULL DEFAULT 10")
    return db
...
def create_assignment(**v) -> Assignment:
    ...
    return Assignment(str(uuid4()), v["title"].strip(), v["course"].strip(),
        ...
        v["category"], v["priority"], v.get("notes", "").strip(), False, int(v.get("points", 10)))
...
def save_assignment(path: Path, x: Assignment) -> None:
    with connect(path) as db:
        db.execute("INSERT OR REPLACE INTO assignments (id,title,course,due_date,due_time,category,priority,notes,completed,points) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (x.id,x.title,x.course,x.due_date,x.due_time,x.category,x.priority,x.notes,int(x.completed),x.points))

def update_assignment(path: Path, item_id: str, **values) -> None:
    allowed={"title","course","due_date","due_time","category","priority","notes","completed","points"}
...
def seed_demo(path: Path) -> None:
    ...
    samples=[("Read chapters 1–3","Media Studies",date(2026,9,22),None,"Reading","Medium","Take notes on visual framing.",10),("Discussion post","Media Studies",date(2026,9,26),"18:00","Assignment","Medium","Respond to two classmates.",15),("Quiz 1","Statistics",date(2026,10,5),"09:30","Quiz","High","Distributions and z-scores.",20),("Research proposal","English 210",date(2026,10,18),"23:59","Project","High","2–3 pages plus sources.",25),("Midterm exam","Statistics",date(2026,11,12),"10:00","Exam","High","Bring calculator and student ID.",30)]
    for title,course,due,t,cat,priority,notes,points in samples: save_assignment(path,create_assignment(title=title,course=course,due_date=due,due_time=t,category=cat,priority=priority,notes=notes,points=points))
