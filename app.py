...
calendar_tab,agenda_tab,manage_tab,syllabi_tab=st.tabs(["Calendar","Agenda","Assignments","Syllabi"])
...
with calendar_tab:
    st.subheader(f"{MONTH_NAMES[month]} {YEAR}"); st.caption("On phones, use the Agenda tab for the clearest view.")
    heads=st.columns(7)
...
with manage_tab:
    ...
    with form_col:
        ...
        with st.form("new",clear_on_submit=True):
            ...
            category=st.selectbox("Type",list(COLORS)); priority=st.selectbox("Priority",["High","Medium","Low"],index=1); notes=st.text_area("Notes",placeholder="Reading pages, submission link, reminders…")
            if st.form_submit_button("Add to calendar",type="primary",use_container_width=True):
                if title.strip() and course.strip(): save_assignment(DB_FILE,create_assignment(title=title,course=course,due_date=due,due_time=due_time if timed else None,category=category,priority=priority,notes=notes)); st.rerun()
